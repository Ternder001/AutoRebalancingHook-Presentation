...
import { PoolMetrics, PositionManager, MarketSimulator, RebalancingVisualizer } from "@/components/dashboard"
import { AutoRebalancingHookABI } from "@/lib/abis/AutoRebalancingHookABI"
import { PoolManagerABI } from "@/lib/abis/PoolManagerABI"

export default function Dashboard() {
  const [provider, setProvider] = useState<ethers.providers.Web3Provider | null>(null)
  const [hookContract, setHookContract] = useState<ethers.Contract | null>(null)
  const [poolManagerContract, setPoolManagerContract] = useState<ethers.Contract | null>(null)
  ...
  const fetchPoolData = async (hook: ethers.Contract, id: string) => {
    try {
      // Get pool state
